...
//        .distinct()
    ...
    def main(args: Array[String]): Unit ={
        ...
        val conf = new SparkConf().setAppName("ItemProfile").setMaster("local[*]").set("spark.sql.shuffle.partitions", "1").set("spark.driver.maxResultSize", "4g")
        ...
        /////////////////// item profiles by date //////////////////////////////////
        def dateToInt(date: String): Int = {
          val arr = date.split("-")
          arr(2).toInt + arr(1).toInt * 32 + arr(0).toInt * 1000
        }
        val dateInteraction: RDD[(Int, Int, Double)] = sc.textFile(args(0))
            .filter(line => !line.startsWith("user_id")) // Skip header
            .map(line => line.split(","))
            .filter(parts => parts.length > 3 && parts(0).forall(_.isDigit) && parts(1).forall(_.isDigit) && parts(3).matches("""\d+(\.\d+)?"""))
            .filter(parts => parts(0).toInt == args(5).toInt && parts(2).split("-").length == 3) // only get the user we want to predict, similarity calculated by date
            .flatMap(parts => Seq((parts(0).toInt, parts(5).toInt, parts(3).toDouble), (2, parts(5).toInt, dateToInt(parts(2)).toDouble)))

        /////////////////// item profiles by nutrition /////////////////////////////
        val userInteraction: RDD[(Int, (Int, Double))] = sc.textFile(args(0))
            .filter(line => !line.startsWith("user_id")) // Skip header
            .map(line => line.split(","))
            .filter(parts => parts.length > 3 && parts(0).forall(_.isDigit) && parts(1).forall(_.isDigit) && parts(3).matches("""\d+(\.\d+)?"""))
            .filter(parts => parts(0).toInt == args(5).toInt)
            .map(parts => (parts(1).toInt, (parts(0).toInt, parts(3).toDouble)))  // item, user, rating

        val nutritionRecipe: RDD[(Int, (Int, Double))] = sc.textFile("C:\\Users\\anhvi\\OneDrive\\Desktop\\Big Data Praktikum\\datasets\\RAW_recipes.csv")
          .filter(line => !line.startsWith("name")) // Skip header
          .filter(line => line.split(",").length >= 5 && line.split("\\[").length >= 3)
          .flatMap(line => { try {
            val itemid = line.split(",")(1).toInt // Ensure the ID is trimmed and converted to Int
            val nutrition = line.split("\\[")(2).split("\\]")(0).replace("\"", "").split(",").map(_.toDouble) // Trim spaces and convert to Double
            Seq((itemid, (1, nutrition(0))), (itemid, (2, nutrition(1))), (itemid, (3, nutrition(2))), 
              (itemid, (4, nutrition(3))), (itemid, (5, nutrition(4))), (itemid, (6, nutrition(5))), 
              (itemid, (7, nutrition(6)))) // item, "user", nutrition
            } catch { case e: Exception => Seq((0, (0, 0.0)))}})
            .filter{ case(i, (j, v)) => j != 0.0}

        val nutritionInteraction = userInteraction.join(nutritionRecipe).
         flatMap{case(key, value) => Seq((value._1._1, key, value._1._2), (value._2._1, key, value._2._2))}
        ///////////////////////////////////////////////////////////////////
        
        
        val input = interactions
        val centeralized = interactions//center(sc, input)
        ...
        val thresh = 2
        val uid = args(5).toInt // 1535, 4740, 2695, 1533
        ...
        val ratingFromUsers = input.filter{ case (user, item, rating) => user == uid }.map{ case (user, item, rating) => (item, rating)}
        ...
        // val comparison = CompleteCosineSimilarityPerRow(sc, input.map{case (r, c, v) => (r, c, v)})
        ...
        // val rmse = math.sqrt(comparison.map{case (k, (r, p, e)) => (e * e)}.sum()) / comparison.count()
        // val avgerr = comparison.map{case (k, (r, p, e)) => e}.sum() / comparison.count()
        // println("\n\n\n\n\n\n\n\n\n\n\n\n")
        // println("RMSE", rmse)
        // println("AVG ERR", avgerr)
        // println("\n\n\n\n\n\n\n\n\n\n\n\n")
